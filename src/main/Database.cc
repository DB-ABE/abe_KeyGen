...
int mysql_generateABEKey(string username, string attibute)
{
    ...
    {
        // 进行用户名和属性的签名
        RSA_Sign(RSA_private_key, username + attibute, RSA_sign_buf, sign_len);
        if (RSA_sign_buf)
        {
            base64String = base64Encode((const unsigned char *)RSA_sign_buf, sign_len);
        }
        ...
        if (strcmp(key->valuestring, "RSA") == 0)
        {
            ...
            abe_cipher.assign(base64String, ret);
            free(base64String);
            base64String = NULL;
            ...
            abe_sign_data.assign(base64String, ret);
            auto sign_ret = RSA_Verify(RSA_ver_key, abe_cipher, (const unsigned char *)abe_sign_data.c_str());
            if (sign_ret != 1)
            {
...
exit:
    ...
    if (request)
        cJSON_Delete(request); // 同时会把key和data free掉
    if (response)
